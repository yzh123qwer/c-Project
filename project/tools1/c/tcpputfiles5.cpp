...
// 如果调用_tcpputfiles()发送了文件，bcontinue为true，初始化为true。
bool bcontinue = true;
...
bool _tcpputfiles() 
{
  ...
  int buflen = 0;  // 用于记录TcpRead函数接收到的报文长度。
  ...
  {
    memset(strrecvbuffer, 0, sizeof(strrecvbuffer));
    if (TcpRead(TcpClient.m_connfd, strrecvbuffer, &buflen, 0))
      break; 
