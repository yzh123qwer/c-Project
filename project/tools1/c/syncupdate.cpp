...
void _help(char *argv[])
{
  printf("Using:/root/project/tools1/bin/syncupdate logfilename xmlbuffer\n\n");

  printf("Example:/root/project/tools1/bin/procctl 10 /root/project/tools1/bin/syncupdate /log/idc/syncupdate_ZHOBTCODE2.log \"<localconnstr>192.168.188.100,root,123456,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTCODE1</fedtname><localtname>T_ZHOBTCODE2</localtname><remotecols>obtid,cityname,provname,lat,lon,height/10,upttime,keyid</remotecols><localcols>stid,cityname,provname,lat,lon,altitude,upttime,keyid</localcols><synctype>1</synctype><timeout>50</timeout><pname>syncupdate_ZHOBTCODE2</pname>\"\n\n");

  // 因为测试的需要，xmltodb程序每次会删除LK_ZHOBTCODE1中的数据，全部的记录重新入库，keyid会变。
  // 所以以下脚本不能用keyid，要用obtid，用keyid会出问题，可以试试。
  printf("        /root/project/tools1/bin/procctl 10 /root/project/tools1/bin/syncupdate /log/idc/syncupdate_ZHOBTCODE3.log \"<localconnstr>192.168.188.100,root,123456,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTCODE1</fedtname><localtname>T_ZHOBTCODE3</localtname><remotecols>obtid,cityname,provname,lat,lon,height/10,upttime,keyid</remotecols><localcols>stid,cityname,provname,lat,lon,altitude,upttime,keyid</localcols><where>where obtid like '54%%%%'</where><synctype>2</synctype><remoteconnstr>192.168.188.100,root,123456,mysql,3306</remoteconnstr><remotetname>T_ZHOBTCODE1</remotetname><remotekeycol>obtid</remotekeycol><localkeycol>stid</localkeycol><maxcount>10</maxcount><timeout>50</timeout><pname>syncupdate_ZHOBTCODE3</pname>\"\n\n");

  printf("       /root/project/tools1/bin/procctl 10 /root/project/tools1/bin/syncupdate /log/idc/syncupdate_ZHOBTMIND2.log \"<localconnstr>192.168.188.100,root,123456,mysql,3306</localconnstr><charset>utf8</charset><fedtname>LK_ZHOBTMIND1</fedtname><localtname>T_ZHOBTMIND2</localtname><remotecols>obtid,ddatetime,t,p,u,wd,wf,r,vis,upttime,keyid</remotecols><localcols>stid,ddatetime,t,p,u,wd,wf,r,vis,upttime,recid</localcols><where>where ddatetime>timestampadd(minute,-120,now())</where><synctype>2</synctype><synctype>2</synctype><remoteconnstr>192.168.188.100,root,123456,mysql,3306</remoteconnstr><remotetname>T_ZHOBTMIND1</remotetname><remotekeycol>keyid</remotekeycol><localkeycol>recid</localkeycol><maxcount>300</maxcount><timeout>50</timeout><pname>syncupdate_ZHOBTMIND2</pname>\"\n\n");
  ...
  printf("maxcount      每批执行一次同步操作的记录数，不能超过MAXPARAMS(256)宏（在_mysql.h中定义），当synctype==2时有效。\n");
...
bool _syncupdate()
{
  ...
  if (starg.synctype==1)
  {
    ...
    if (stmtins.execute()!=0)
    {
      ...
      connloc.rollback();   // 如果这里失败了，一定要回滚事务。撤销上一步删除本地表的数据。
      return false;
    }
  ...
  // 准备删除本地数据表的SQL语句，一次删除starg.maxconut条记录。
  ...
  while(true)
  {
    ...
    if (ccount==starg.maxcount) // 数据条数大于maxcount
    {
      // 从本地表中删除记录。
      if (stmtdel.execute()!=0)
      {
