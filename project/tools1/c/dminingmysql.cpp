...
int ifieldlen[MAXFIELDLEN];           // 结果集字段的长度数组，从starg.fieldlen解析得到。
int ifieldcount;                       // strfieldname和ifieldlen中的有效字段个数。
...
connection conn;  // 用于连接数据源数据库。
connection conn1; // 用于连接存放自增字段最大值的数据库。

char strxmlfilename[301]; // xml文件名。

long imaxincvalue; // 自增字段中的最大值。
...
bool _dminingmysql()
{
  ...
  char strfieldvalue[ifieldcount][MAXFIELDLEN + 1]; // 抽取数据的SQL执行后，存放结果集字段的数值。
  for (int ii = 1; ii < ifieldcount + 1; ii++)
  {
  ...
  while(true)
  {
    ...
    // 如果记录数达到starg.maxcount行就切换一个新的文件。
...
bool _xmltoarg(char *strxmlbuffer)
{
  ...
  {
    logfile.Write("fieldlen字段数太多，超出了最大限制%d。\n", MAXFIELDCOUNT);
    return false;
  }
  ...
  for (int ii=0; ii<CmdStr.CmdCount(); ii++)
  {
    ...
    if (ifieldlen[ii] > MAXFIELDLEN)
      ifieldlen[ii] = MAXFIELDLEN; // 字段的长度不能超过MAXFIELDLEN。
                                   // 可能导致数据截断，但是防止内存溢出。
  }
...
bool readincfield()
{
  ...
  if (strlen(starg.connstr1) != 0)
  {
    ...
    if (stmt.execute() != 0)
logfile.Write("inc excute failed.\n");
    stmt.next();
  }
  else
  {
...
bool writeincfield()
{
  ...
  if (strlen(starg.connstr1) != 0)
  {
    ...
    if (stmt.m_cda.rc==1146) // 执行prepare是与数据库有交互的。
    {
      // 如果表不存在，就创建表。
