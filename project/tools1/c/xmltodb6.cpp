...
struct st_xmltotable     // 参数文件的保存的结构体。
{
...
bool _xmltodb()
{
  ...
  while(true)
  {
    ...
    {
      counter = 0;  // 重新计数。
      // 把数据入库的参数文件配置文件starg.inifilename加载到容器中。预防入库参数的配置文件修改。
    ...
    while (true)
    {
      ...
      if ((iret==1) || (iret == 2))    // iret==1，找不到入库参数。
      {
        if (iret == 1)
...
int _xmltodb(char *fullfilename,char *filename)
{
  ...
  // 在处理xml文件之前，如果stxmltotable.execsql，就执行它。
  if (execsql() == false)
    return 6;

  // 打开xml文件。
  CFile File;
  if (File.Open(fullfilename, "r") == false)  // 打开xml文件失败。
  {
    conn.rollback();
    return 3;
  }
...
bool loadxmltotable()
{
  ...
  while (true)
  {
    ...
    GetXMLBuffer(strBuffer,"uptbz",&stxmltotable.uptbz);          // 更新标志：1-更新；2-不>更新。
...
void crtsql()
{
  ...
  {
    // upttime（系统当前时间）和keyid（自增字段）这两个字段不需要处理。
